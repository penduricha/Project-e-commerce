...
public interface ProductRepository extends JpaRepository<Product,Long> {
    @Query("select new com.example.project_e_commerce_backend.dtos.ProductDto(p.name, p.description, p.status, pt.typeProduct) " +
            "from Product p " +
            "left join p.productType pt " +
            "order by p.productId")
    List<ProductDto> getAllProductAndType();
}
