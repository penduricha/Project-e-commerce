...
export default{
  ...
  created(){
    this.getProductExplore_Products_From_API();
  },

  methods:{
    async getProductExplore_Products_From_API(){
      const productDao = new ProductDao();
      let products = [];
      try{
        products = await productDao.getExploreOurProducts();
        this.products_Explore_Our_Products = products;
        let products_View = await products;
        console.log('Product Flash Sales: ',products_View);
        this.displayedProducts = this.products_Explore_Our_Products.slice(0, this.productsToShow);
      }catch(e){
        console.error(e);
        alert(e);
      }
    ...
    async toggleViewAll() {
      if (this.productsToShow < this.products_Explore_Our_Products.length) {
    ...
    <main class="main">
      <div class="view-product-slide" >
        ...
        <div class="view-list-product">
          ...
          <div style="width: 100%; height: 56px; display: flex; justify-content: center; align-items: center;" v-if="productsToShow <= products_Explore_Our_Products.length">
